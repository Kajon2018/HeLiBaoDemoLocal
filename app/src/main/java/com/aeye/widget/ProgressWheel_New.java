 ...
 */
public class ProgressWheel_New extends View {
  private static final String TAG = ProgressWheel_New.class.getSimpleName();
   ...
   */
  public ProgressWheel_New(Context context, AttributeSet attrs) {
    super(context, attrs);

    parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.ProgressWheel1));
   ...
   * The constructor for the ProgressWheel
   */
  public ProgressWheel_New(Context context) {
    super(context);
    setAnimationEnabled();
  }
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius =
        (int) a.getDimension(R.styleable.ProgressWheel1_matProg_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.ProgressWheel1_matProg_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.ProgressWheel1_matProg_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.ProgressWheel1_matProg_rimWidth, rimWidth);

    float baseSpinSpeed =
        a.getFloat(R.styleable.ProgressWheel1_matProg_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
        a.getInt(R.styleable.ProgressWheel1_matProg_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.ProgressWheel1_matProg_barColor, barColor);

    rimColor = a.getColor(R.styleable.ProgressWheel1_matProg_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.ProgressWheel1_matProg_linearProgress, false);

    if (a.getBoolean(R.styleable.ProgressWheel1_matProg_progressIndeterminate, false)) {
      spin();
    }
