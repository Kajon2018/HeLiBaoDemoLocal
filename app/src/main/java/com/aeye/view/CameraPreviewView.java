...
public class CameraPreviewView extends SurfaceView implements 
	...
	private AuthenActivity mActivity = null;
	...
	public void setActivity(AuthenActivity activity) {
		mActivity = activity;
	}
	...
	public void run() {
		while (mRun) {
			...
			if(canvas != null) {
				...
				Bitmap nbmp =setScale(bmp);
				bmp.recycle();
				bmp=null;
//				BitmapUtils.saveUserPicture(bmp, "a"+System.currentTimeMillis());
				canvas.drawBitmap(nbmp, null, new RectF(mX, mY, mX+bmpWidth, mY+bmpHeight),
						null);
				nbmp.recycle();
				nbmp = null;
